...
# We will mock having a state or database for this development server byt setting a global variable. You would never use a global variable in a production server.
...
class Home
  # ActiveModel is part of Ruby on Rails. It is used as an ORM. It has a module within ActiveModel that provides validations. The production Terratowns server is rails and uses very similar and in most cases identical validation.
  # https://guides.rubyonrails.org/active_model_basics.html
  # https://guides.rubyonrails.org/active_record_validations.html
  include ActiveModel::Validations
  # Create some virtual attributes to stored on this object
  # This will set a getter and setter, for example:
  # home = new Home()
  ...
  # Content version has to be an integer. We will make sure it an incremental version in the controller:
...
class TerraTownsMockServer < Sinatra::Base
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    # BEGIN/RESCUE is a try/catch. So if an error occurs, rescue it.
    begin
      # Sinatra does not automatically parse json bodys as params like rails, so we need to manuall parse it:
    ...
    puts "uuid #{uuid}"
    # Will mock save our data to our mock databse, which is just a global variable:
    $home = {
      uuid: uuid,
      name: name,
      town: town,
  ...
  # UPDATE
  # Very similar to the CREATE action above
